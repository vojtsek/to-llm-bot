...
export OPENAI_API_KEY=`cat ~/.openai-agnostix`
...
if [[ "${dataset}" == "sgd" ]]; then
    ...
    python run.py --model_name $MODEL --faiss_db mw-context-2-50perdomain.vec --num_examples 2 --database_path multiwoz_database --context_size 2 --output results.txt --dataset multiwoz $@
fi
